...
Let's start with matrices (the plural of matrix).
...
:::{.callout-note}
A `matrix` is an `array` with two dimensions. We could also create arrays with an arbitrary number of dimensions, but these are rarely needed in statistical data analysis pipelines.
:::
...
A matrix is most useful for purely numerical data. In practice, we often need to deal with non-numerical data such as categorical variables. Since matrices are vectors, they can only consist of elements with the same data type. Therefore, we cannot use them to represent heterogeneous data consisting of columns with different data types.


## Data frames

Like matrices, data frames are two-dimensional data structures consisting of rows and columns. In contrast to a matrix, a data frame can contain columns with different data types. For example, a data frame can consist of a mixture of numerical columns, character columns, and so on. We can think of a data frame as a collection of columns, where each column is represented by a vector.
...
The function `data.frame()` creates a data frame from a vectors, which are usually passed as named arguments as follows:
...
Although `rownames()` works with data frames, renaming rows is rarely ever needed and should be avoided. The default sequential row names are perfectly fine for almost all use cases, so let's restore them here:
...
:::{.callout-tip}
We already know that under the hood, a data frame is a list of vectors. Therefore, as an alternative to the `$` syntax we can also access individual list elements (columns) as follows (note the double square brackets):
...
Data frames are one of the most widely used data types in R. However, getting a quick and clear overview of their contents can be cumbersome for two reasons. First, R prints way too many rows – usually much more rows than would fit comfortably in the R console. The same is true if a data frame has many columns, R will usually print all columns even if they do not fit within the width of the R console.
...
Another minor annoyance with data frames is that column types are only displayed when explicitly calling `str()` – they are not shown in the representation of the data frame.
...
Finally, `View()` creates a spreadsheet-like view of the data, which is useful for interactive exploration:

```r
View(co2_tibble)  # opens a spreadsheet
```

Note that the spreadsheet is read-only, so this view cannot be used to edit values of a data frame.
