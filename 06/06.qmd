...
As an example, we import the file [`lecturer.csv`](lecturer.csv) from the previous chapter:
...
We no longer need the `name` column for our subsequent considerations, so we drop it:
...
The `summary()` function provides a useful summary for each column of a data frame. Numerical columns and date columns are described with six values:
...
The `summary()` function also works for other column types and often provides a useful short description. Noteably, it reports the number of missing values per column (if any).
...
:::{.callout-note}
We can use functions from packages without activating the package. To do this, we have to prefix the package name followed by `::`. In the previous example, we could therefore omit `library(psych)` and still use the function with `psych::describe()`. In either case, the package must be installed on your system.
:::
...
However, this does not guarantee that all numbers will be displayed in decimal notation. Another option is to display numbers as normal decimal numbers using `format()`. This always results in a character vector, but does not require changing an option:
...
The `stat.desc()` function already provides the result of the [Shapiro-Wilk test](https://en.wikipedia.org/wiki/Shapiro%E2%80%93Wilk_test) for normality (the entries `normtest.W` and `normtest.p` contain the test statistic and the significance, respectively). If `normtest.p` is significant (e.g., less than 0.05), the null hypothesis of normality can be rejected. You can also call the Shapiro-Wilk test directly with the `shapiro.test()` function:
...
4. Load the dataset [`lecturer.dat`](lecturer.dat) and calculate the means of all numerical columns grouped by the column `job` using the `by()` function. Why does this calculation not work when passing the `mean` function as an argument to `by()`?
